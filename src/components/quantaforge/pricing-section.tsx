...
const tiers = [{
  name: 'Starter (Pilot)',
  price: 'from $85k',
  features: ['Single backend compiler sprint', '1 target workflow (e.g., VQE)', 'Benchmarking report', '8 weeks support'],
  cta: 'Get Started'
}, {
  name: 'Growth (Multi‑backend)',
  price: 'from $240k',
  features: ['IR + 3 backends', 'CI portability suite', 'PQC readiness blueprint', '3 months support'],
  cta: 'Contact Sales'
}, {
  name: 'Enterprise (Residency)',
  price: 'POA',
  features: ['Dedicated team (compiler + crypto + simulation)', 'SLOs', 'Co‑authored publications', 'Regulator packages'],
  cta: 'Book a Scoping'
}];
const addOns = ['PQC rollout', 'MPC wallet module', 'QKD peering', 'Pulse‑level optimizer', 'Training'];
export const PricingSection = () => {
  return <div id="pricing-section" className="py-12 sm:py-16 md:py-20 lg:py-24 xl:py-32 bg-quantaforge-dark">
      <div className="container mx-auto px-4 sm:px-6 md:px-8 my-0">
        ...
        <div className="mt-12 md:mt-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
          {tiers.map(tier => <div key={tier.name} className="flex flex-col rounded-3xl bg-quantaforge-dark/50 ring-1 ring-inset ring-quantaforge-light/10">
        ...
        <div className="mt-12 md:mt-16 text-center">
          ...
          <div className="mt-4 md:mt-6 flex flex-wrap justify-center gap-3 md:gap-4">
            {addOns.map(addOn => <span key={addOn} className="px-4 py-2 text-sm text-quantaforge-light/80 bg-quantaforge-dark/50 rounded-full ring-1 ring-inset ring-quantaforge-light/10">
                {addOn}
              </span>)}
          </div>
        </div>
