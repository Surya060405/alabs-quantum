      ...
      <div className="container mx-auto px-4 sm:px-6 md:px-8">
        ...
        >
          <h3 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-4 md:mb-6 text-quantaforge-secondary flex items-center justify-center gap-2">
            Introducing
            <img src={TransparentLogo} alt="Quantum Labs Logo" className="h-12 mt-2 sm:h-10 md:h-12 w-auto" />
          </h3>
          <p className="text-sm md:text-base text-quantaforge-light/70 leading-relaxed max-w-3xl mx-auto">
            ...
            Labs) helps engineering leaders ship credible quantum initiatives with lean teams and predictable ROI. Our
            technology stack spans Quantum Simulation & AI, Quantum Compiler Studio, and Quantum‑Safe Tokenization,
            integrating with your GPUs/HPC, existing ML frameworks, and major quantum backends. You gain faster
            iteration, lower manpower cost, and a secure path to production and PQC migration—without a hiring spike.
            Founded by accomplished high tech veterans from different domains, Quantum Labs aspires to be a trusted
            solution partner for emerging quantum entrepreneurs and R&D teams. For more information about Quantum Labs
            and its novel solutions in the quantum landscape, please visit:{" "}
            <a
              href="https://www.alabs-quantum.com"
              target="_blank"
              rel="noopener noreferrer"
              className="text-quantaforge-secondary hover:underline"
            >
              www.alabs-quantum.com
            </a>{" "}
            or contact us via email:{" "}
            <a href="mailto:info@alabs-quantum.com" className="text-quantaforge-secondary hover:underline">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-6 max-w-6xl mx-auto">
            {services.map((service, index) => (
              ...
              >
                ...
                <p className="text-xs md:text-sm text-quantaforge-light/70">{service.description}</p>
              </motion.div>
            ))}
          </div>
        </motion.div>
