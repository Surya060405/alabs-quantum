import { motion } from "framer-motion";
import { Cpu, Settings, ArrowLeft, Check } from "lucide-react";
import { Link } from "react-router-dom";
import { Navigation } from "@/components/navigation";
import { Footer } from "@/components/footer";

const VlsiEdaCad = () => {
  const designCapabilities = [
    "Advanced CPU, GPU, NPU Architecture",
    "Low power high performance design strategies",
    "ASIC, Semiconductor Process",
    "Digital/Analog and Mixed-signal design",
    "AI-powered chips, DLA",
    "Custom, Memory, Digital/Analog RF, Std. Cell",
    "CPU/SOC/ASIC, RTL, Func/Formal Verification",
    "Synthesis, PD (RTL2GDS-II), Sign off",
    "Timing & Power, LPD, Process corners"
  ];

  const edaCapabilities = [
    "Vendor SW (Synopsys, Cadence, Mentor)",
    "FE, RTL, Func/Formal/AMS, Ckt Sim, Synth",
    "BE, FP, P&R, CTS, ParExt, STA, Power, LVS/DRC",
    "Math/Stat optim, C/C++, Perl, Python, TCL"
  ];

  const technologies = [
    "Synopsys", "Cadence", "Mentor Graphics", "RTL Design", "Verilog", "VHDL",
    "SystemVerilog", "UVM", "Synthesis", "P&R", "STA", "DRC", "LVS",
    "C/C++", "Python", "Perl", "TCL", "ASIC", "FPGA"
  ];

  return (
    <div className="min-h-screen bg-background">
      <Navigation />
      
      {/* Hero Section */}
      <section className="relative pt-32 pb-20 overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-neon-purple/10 via-transparent to-neon-green/10" />
        <div className="container mx-auto px-4 relative z-10">
          <Link to="/" className="inline-flex items-center text-muted-foreground hover:text-neon-purple transition-colors mb-8">
            <ArrowLeft className="w-4 h-4 mr-2" />
            Back to Home
          </Link>
          
          <motion.div
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6 }}
            className="text-center max-w-4xl mx-auto"
          >
            <div className="inline-flex gap-4 mb-6">
              <div className="inline-block p-4 rounded-2xl bg-neon-purple/10">
                <Cpu className="w-16 h-16 text-neon-purple" />
              </div>
              <div className="inline-block p-4 rounded-2xl bg-neon-green/10">
                <Settings className="w-16 h-16 text-neon-green" />
              </div>
            </div>
            <h1 className="text-5xl lg:text-6xl font-bold mb-6">
              <span className="text-gradient-purple">VLSI IC Design & EDA/CAD</span>
            </h1>
            <p className="text-xl text-muted-foreground leading-relaxed">
              Complete integrated circuit design solutions from concept to silicon, including electronic design automation tools and custom CAD flows
            </p>
          </motion.div>
        </div>
      </section>

      {/* Overview Section */}
      <section className="py-20 relative">
        <div className="container mx-auto px-4">
          <motion.div
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6 }}
            viewport={{ once: true }}
            className="glassmorphism p-10 rounded-2xl max-w-5xl mx-auto"
          >
            <h2 className="text-3xl font-bold text-gradient-purple mb-6">Overview</h2>
            <p className="text-lg text-muted-foreground leading-relaxed mb-6">
              Our VLSI team delivers full-cycle integrated circuit design services, from advanced CPU/GPU/NPU architectures 
              to complete RTL-to-GDS-II implementation. We specialize in high-performance, low-power designs for AI chips, 
              ASICs, and custom silicon solutions.
            </p>
            <p className="text-lg text-muted-foreground leading-relaxed">
              With expertise in industry-leading EDA tools from Synopsys, Cadence, and Mentor Graphics, we provide 
              comprehensive solutions spanning frontend RTL design, functional/formal verification, synthesis, physical design, 
              and signoff. Our custom CAD flows optimize for power, performance, and area across all process corners.
            </p>
          </motion.div>
        </div>
      </section>

      {/* Technologies Section */}
      <section className="py-20 relative">
        <div className="absolute inset-0 bg-gradient-to-br from-neon-green/5 via-transparent to-neon-purple/5" />
        <div className="container mx-auto px-4 relative z-10">
          <motion.div
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6 }}
            viewport={{ once: true }}
            className="text-center mb-12"
          >
            <h2 className="text-4xl font-bold mb-4">
              <span className="text-gradient-purple">Technologies & Tools</span>
            </h2>
            <p className="text-xl text-muted-foreground">
              Industry-standard EDA tools and methodologies
            </p>
          </motion.div>

          <motion.div
            initial={{ opacity: 0 }}
            whileInView={{ opacity: 1 }}
            transition={{ duration: 0.8, delay: 0.2 }}
            viewport={{ once: true }}
            className="flex flex-wrap justify-center gap-4 max-w-5xl mx-auto"
          >
            {technologies.map((tech, index) => (
              <motion.div
                key={index}
                initial={{ opacity: 0, scale: 0.8 }}
                whileInView={{ opacity: 1, scale: 1 }}
                transition={{ duration: 0.4, delay: index * 0.05 }}
                viewport={{ once: true }}
                whileHover={{ scale: 1.05 }}
                className="glassmorphism px-6 py-3 rounded-full border border-neon-purple/20 hover:border-neon-purple/50 transition-all duration-300"
              >
                <span className="text-foreground font-medium">{tech}</span>
              </motion.div>
            ))}
          </motion.div>
        </div>
      </section>

      {/* IC Design Capabilities Section */}
      <section className="py-20">
        <div className="container mx-auto px-4">
          <motion.div
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6 }}
            viewport={{ once: true }}
            className="text-center mb-12"
          >
            <h2 className="text-4xl font-bold mb-4">
              <span className="text-gradient-blue">IC Design Capabilities</span>
            </h2>
          </motion.div>

          <div className="max-w-5xl mx-auto grid md:grid-cols-2 gap-6">
            {designCapabilities.map((capability, index) => (
              <motion.div
                key={index}
                initial={{ opacity: 0, x: -20 }}
                whileInView={{ opacity: 1, x: 0 }}
                transition={{ duration: 0.5, delay: index * 0.1 }}
                viewport={{ once: true }}
                className="glassmorphism p-6 rounded-xl flex items-start gap-4 hover:glow-neon-purple transition-all duration-300"
              >
                <div className="flex-shrink-0 w-6 h-6 rounded-full bg-neon-purple/20 flex items-center justify-center mt-1">
                  <Check className="w-4 h-4 text-neon-purple" />
                </div>
                <p className="text-muted-foreground leading-relaxed">{capability}</p>
              </motion.div>
            ))}
          </div>
        </div>
      </section>

      {/* EDA/CAD Capabilities Section */}
      <section className="py-20 relative">
        <div className="absolute inset-0 bg-gradient-to-br from-neon-purple/5 via-transparent to-neon-green/5" />
        <div className="container mx-auto px-4 relative z-10">
          <motion.div
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6 }}
            viewport={{ once: true }}
            className="text-center mb-12"
          >
            <h2 className="text-4xl font-bold mb-4">
              <span className="text-gradient-green">EDA/CAD Capabilities</span>
            </h2>
          </motion.div>

          <div className="max-w-5xl mx-auto grid md:grid-cols-2 gap-6">
            {edaCapabilities.map((capability, index) => (
              <motion.div
                key={index}
                initial={{ opacity: 0, x: -20 }}
                whileInView={{ opacity: 1, x: 0 }}
                transition={{ duration: 0.5, delay: index * 0.1 }}
                viewport={{ once: true }}
                className="glassmorphism p-6 rounded-xl flex items-start gap-4 hover:glow-neon-green transition-all duration-300"
              >
                <div className="flex-shrink-0 w-6 h-6 rounded-full bg-neon-green/20 flex items-center justify-center mt-1">
                  <Check className="w-4 h-4 text-neon-green" />
                </div>
                <p className="text-muted-foreground leading-relaxed">{capability}</p>
              </motion.div>
            ))}
          </div>
        </div>
      </section>

      {/* CTA Section */}
      <section className="py-20 relative overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-neon-purple/10 via-transparent to-neon-green/10" />
        <div className="container mx-auto px-4 relative z-10">
          <motion.div
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6 }}
            viewport={{ once: true }}
            className="glassmorphism p-12 rounded-2xl text-center max-w-4xl mx-auto"
          >
            <h2 className="text-3xl font-bold text-gradient-purple mb-6">
              Design Your Next Chip with Us
            </h2>
            <p className="text-xl text-muted-foreground mb-8">
              From concept to silicon, let's build cutting-edge integrated circuits together
            </p>
            <Link
              to="/#contact"
              className="inline-block bg-primary hover:bg-primary/80 text-primary-foreground px-8 py-4 rounded-lg font-semibold text-lg glow-neon-purple transition-all duration-300"
            >
              Get Started
            </Link>
          </motion.div>
        </div>
      </section>

      <Footer />
    </div>
  );
};

export default VlsiEdaCad;
